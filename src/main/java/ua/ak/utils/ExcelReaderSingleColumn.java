...
//single column version refactored
public class ExcelReaderSingleColumn {
	...
	public ExcelReaderSingleColumn(String FILE_NAME) {
	...
	public List<FieldOperation> getAllOperations() {
		...
		try {
			...
			for (int i = ROW_HEADER + 1; i < MAX_ROW_FROM_EXCEL; i++) {
				...
				if (!((seedsType.equals(null) || seedsType.equals("") || seedsUsageQty == 0))) {

					if (tmpFuelSplitter > 0) {
						fuelLiters = 0;
					}

					listFieldOperation.add(new FieldOperation(date, fieldCode, fiedArea, operation, doneHa, tractor, registrationNumber,
							tractordriver, motorHours, equipment, serialNumber, fuelLiters, crop, seedsType, seedsUsageQty, year));
					tmpFuelSplitter++;

				}

				if (! ((fertilizerType.equals(null) || fertilizerType.equals("") || fertilizerUsageQty == 0))) {
					if (tmpFuelSplitter > 0) {
						fuelLiters = 0;
					}
					listFieldOperation.add(new FieldOperation(date, fieldCode, fiedArea, operation, doneHa, tractor, registrationNumber,
							tractordriver, motorHours, equipment, serialNumber, fuelLiters, crop, fertilizerType, fertilizerUsageQty, year));
					tmpFuelSplitter++;
				}

				if ((!(chemicalsType.equals(null) || chemicalsType.equals("") || chemicalsUsageQty == 0))) {

					if (tmpFuelSplitter > 0) {
						fuelLiters = 0;
					}
					listFieldOperation.add(new FieldOperation(date, fieldCode, fiedArea, operation, doneHa, tractor, registrationNumber,
							tractordriver, motorHours, equipment, serialNumber, fuelLiters, crop, chemicalsType, chemicalsUsageQty, year));
					tmpFuelSplitter++;
				}
				tmpFuelSplitter=0;
